...
- **Codex** → os comandos podem começar com **`@`** (ex.: `@avatar-funil`) e as skills ficam em **`.agents/skills/`**. Se o aluno estiver no Codex e as skills não aparecerem, verifique se a pasta `.agents/skills/` existe; se as skills só estão em `.claude/skills/`, oriente a copiar/linkar pra `.agents/skills/` (posso fazer isso pra você — pergunte antes).
...
- **Não apareceram nada**: não trave. Cheque se você (Claude/Codex) está rodando **dentro da pasta do projeto** (o `ls` do Passo 1 mostrando `.claude/` confirma). No Codex, confirme que as skills estão em `.agents/skills/` (ver 2b). Se ainda assim não vierem, peça pro aluno fechar e reabrir a ferramenta na pasta e rodar `/comecar` de novo — a gente recomeça sem susto.
